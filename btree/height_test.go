...
func TestHeight_Should_Return_Correct_Height(t *testing.T) {
	tests := []struct {
		...
		{
			tree:     NewBtreeWithPager(3, NewMemPager(&PersistentKeySerializer{}, &StringValueSerializer{Len: 5})),
		...
		{
			tree:     NewBtreeWithPager(4, NewMemPager(&PersistentKeySerializer{}, &StringValueSerializer{Len: 5})),
			toInsert: []PersistentKey{PersistentKey(1), PersistentKey(2), PersistentKey(3), PersistentKey(4)},
			expected: 2,
		},
		{
			tree:     NewBtreeWithPager(5, NewMemPager(&PersistentKeySerializer{}, &StringValueSerializer{Len: 5})),
