...
func TestPersistentInsert_Or_Replace_Should_Return_False_When_Key_Exists(t *testing.T) {
	...
	tree := NewBtreeWithPager(transaction.TxnNoop(), 80, NewTestBPP(pool, &PersistentKeySerializer{}, io.Discard))
	for i := 0; i < 1000; i++ {
...
func TestPersistentEvery_Inserted_Should_Be_Found_VarSized(t *testing.T) {
	...
	pool := buffer.NewBufferPool(dbName, 16, wal.NoopLM)
	tree := NewBtreeWithPager(transaction.TxnNoop(), 32, NewBPP(pool, &StringKeySerializer{}, &StringValueSerializer{}, wal.NoopLM))
	log.SetOutput(io.Discard)
	n := 100000
	for _, i := range rand.Perm(n) {
...
func TestPersistent_Pin_Count_Should_Be_Zero_After_Inserts_Are_Complete(t *testing.T) {
	...
	pool := buffer.NewBufferPool(dbName, 5, wal.NoopLM)
	tree := NewBtreeWithPager(transaction.TxnNoop(), 10, NewTestBPP(pool, &PersistentKeySerializer{}, io.Discard))
	log.SetOutput(io.Discard)
	n := 1000
	for _, i := range rand.Perm(n) {
...
func TestPersistent_All_Inserted_Should_Be_Found_After_File_Is_Closed_And_Reopened(t *testing.T) {
	...
	newPool := buffer.NewBufferPoolV2WithDM(false, poolSize, dm, wal.NoopLM)
	newTreeReference := ConstructBtreeByMeta(tree.metaPID, NewTestBPP(newPool, &PersistentKeySerializer{}, io.Discard))
